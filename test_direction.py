    ...
    for key, value in members.items():
        if 'cd' in value:
            ...
            if re.search('910', goal) or re.search('01', goal) or re.search('计', goal) and re.search('专', goal):
                pro_910 += 1
                pro += 1
            elif re.search('保密', goal) and re.search('学', goal) or re.search('940', goal) and re.search('学',
                                                                                                        goal) or re.search(
                    '网', goal) and re.search('学', goal):
                ac_940 += 1
                ac += 1
            elif re.search('940', goal) or re.search('保密', goal) or re.search('02', goal) or re.search('网', goal):
                pro_940 += 1
                pro += 1
            elif re.search('912', goal) or re.search('软', goal) and re.search('学', goal) or re.search('软',
                                                                                                      goal) and re.search(
                    '理', goal):
                ac_912 += 1
                ac += 1
                ...
                pro += 1
                # alias.append(goal)
            elif re.search('954', goal) or re.search('计', goal) and re.search('学', goal) or re.search('计',
                                                                                                      goal) and re.search(
                    '应', goal) or re.search('计', goal) and re.search('科', goal):
                ac_954 += 1
                ac += 1
                ...
                other += 1
    res = [ratio(pro_910, 40), ratio(pro_911, 38), ratio(pro_940, 22), ratio(ac_912, 12), ratio(ac_940, 7),
           ratio(ac_954, 27), ratio(pro, ac), pro + ac + other, pro + ac, ratio(pro_930_341, 11)]
