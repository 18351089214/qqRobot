...
async def wechat_process(client, loop):
    ...
    json_post = lambda api, params, data: client.post(base_uri + api, params=params,
    ...
    data = dict(BaseRequest=base_request, Code=3, FromUserName=user['UserName'], ToUserName=user['UserName'],
    ...
    for synchost in ('webpush.weixin.qq.com', 'webpush2.weixin.qq.com', 'webpush.wechat.com', 'webpush1.wechat.com',
    ...
    async def sendmsg(cmd):
        ...
        data = dict(BaseRequest=base_request,
                    Msg=dict(Type=1, Content=content, FromUserName=user['UserName'], ToUserName=c['UserName'],
                             LocalID=msgid, ClientMsgId=msgid))
